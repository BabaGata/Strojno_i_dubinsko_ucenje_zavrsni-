...
class PreprocessCSVtoSequences():
    ...
    def preprocess(self):
        ...
        df['Change_Column'] = df.progress_apply(
        ...
        for _, row in tqdm(df.iterrows(), total=df.shape[0]):
            row_data = dict(
                ...
                high = row['High'],
                low = row['Low'],
                close_change = row['Change_Column'],
        ...
        self.train_sequences = self.create_sequences(train_df, 'close', self.sequence_length)
        self.val_sequences = self.create_sequences(val_df, 'close', self.sequence_length)
        self.test_sequences = self.create_sequences(test_df, 'close', self.sequence_length)
